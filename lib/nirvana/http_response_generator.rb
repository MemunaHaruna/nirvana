  ...
  class HTTP_response_generator
    ...
    def parse
      ...
      env = {
        ...
        "SERVER_PORT" => uri.port || 8888,
        "SERVER_NAME" => uri.host || "localhost",
        "PATH_INFO" => uri.path || "",
        "SCRIPT_NAME" => "",
      }

      while matches = /\A(?<key>[^:]+):\s*(?<value>.+)#{CRLF}\Z/.match(@socket.gets)
        case matches[:key]
    ...
    def handle
      ...
      Nirvana.logger.log("[#{time}] '#{env["REQUEST_METHOD"]} #{env["REQUEST_URI"]} #{env["HTTP_VERSION"]}' #{status}")
    end
  end
end
